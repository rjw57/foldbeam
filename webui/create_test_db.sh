...
function post_file() {
    ...
    URL=`http -p h POST $1 contents@$2 | grep '^Location:' | sed -e 's/^Location:\s*//'`
    if [ -z "${URL}" ]; then
        echo "Error loading $1" >&2
        exit 1
    fi
...
post_file ${B1}/foo.shp ../data/ne_110m_admin_0_countries.shp
post_file ${B1}/foo.shx ../data/ne_110m_admin_0_countries.shx
post_file ${B1}/foo.dbf ../data/ne_110m_admin_0_countries.dbf
post_file ${B1}/foo.prj ../data/ne_110m_admin_0_countries.prj

B2=`post http://localhost:8888/user1/bucket name="tiff_test"`
post_file ${B2}/input.tiff ../data/spain.tiff

B3=`post http://localhost:8888/user1/bucket name="png_test_1"`
post_file ${B3}/input.png ../data/spain.png

B4=`post http://localhost:8888/user1/bucket name="png_test_2"`
post_file ${B4}/input.png ../data/spain.png
post_file ${B4}/input.png.aux.xml ../data/spain.png.aux.xml
