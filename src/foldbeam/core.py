...
def reproject(srcs, dst):
    if len(srcs) == 0:
        return

    dst_x = np.linspace(dst.envelope.left, dst.envelope.right, dst.array.shape[1])
    dst_y = np.linspace(dst.envelope.top, dst.envelope.bottom, dst.array.shape[0])

    dst_x = np.repeat(np.atleast_2d(dst_x), dst.array.shape[0], 0)
    dst_y = np.repeat(np.atleast_2d(dst_y).transpose(), dst.array.shape[1], 1)

    assert dst_x.shape[:2] == dst.array.shape[:2]
    assert dst_y.shape[:2] == dst.array.shape[:2]

    src_srs = srcs[0].envelope.spatial_reference
    dst_srs = dst.envelope.spatial_reference

    assert all([src.envelope.spatial_reference.IsSame(src_srs) for src in srcs[1:]])

    if src_srs.IsSame(dst_srs):
        src_x, src_y = dst_x, dst_y
    else:
        src_proj = pyproj.Proj(src_srs.ExportToProj4())
        dst_proj = pyproj.Proj(dst_srs.ExportToProj4())
        assert src_proj is not None
        assert dst_proj is not None
        src_x, src_y = pyproj.transform(dst_proj, src_proj, dst_x, dst_y)

    assert src_x.shape[:2] == dst.array.shape[:2]
    assert src_y.shape[:2] == dst.array.shape[:2]
    
    for src in srcs:
        # Convert source x and y into pixel co-ordinates
        src_xp = (src_x - src.envelope.left) * ((src.array.shape[1]-1) / src.envelope.offset()[0])
        src_yp = (src_y - src.envelope.top) * ((src.array.shape[0]-1) / src.envelope.offset()[1])

        src_xp = np.int32(np.round(src_xp))
        src_yp = np.int32(np.round(src_yp))
        valid_x = np.logical_and(src_xp >= 0, src_xp < src.array.shape[1])
        valid_y = np.logical_and(src_yp >= 0, src_yp < src.array.shape[0])
        valid = np.logical_and(valid_x, valid_y)

        src_xp = np.clip(src_xp, 0, src.array.shape[1]-1)
        src_yp = np.clip(src_yp, 0, src.array.shape[0]-1)

        for i in xrange(4):
            dst.array[:,:,i] = np.where(valid, src.array[src_yp, src_xp, i], dst.array[:,:,i])
